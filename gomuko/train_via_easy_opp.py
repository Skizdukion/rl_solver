...
def forward_pass(envs, agent, num_rollout_steps, discount_factor):
    ...
    actions_log_probability = torch.stack(actions_log_probability).to(device)
    values = torch.stack(values).to(device)
    rewards_tensor = torch.stack(rewards).to(device)
    dones_tensor = torch.stack(dones).to(device)

    advantages, returns = calculate_gae(
        rewards_tensor, values, dones_tensor, last_value
    )
